...
def writeaccesslog(u):
    ...
    with open(accesslog, mode="a", encoding="UTF-8") as f:
        f.write(
            f"Access at time: {datetime.datetime.now():%Y-%m-%d %H:%M:%S} by user: {u}\n"
        )
...
def intro():
    ...
    sleep(2)
    repeat = input(
        "Would you like to ADD, SEARCH, EDIT or VIEW a country(type as shown, n to skip)? "
    )
    sleep(1)
    while repeat != "n":
        ...
        repeat = input(
            "Would you like to ADD, SEARCH, EDIT or VIEW another country(type as shown, n to skip)? "
        )
    sleep(1)
    ...
    for i in events.keys():
        print(
            f'{i:<30} {events.get(i).strftime("%Y-%m-%d"):<14} {events.get(i).strftime("%H:%M"):>20}'
        )
    sleep(1)
